...
Box::Box(Scene *scene, int pox, int poy, int wih, int heit, std::string path)
    : GameObject(scene), posx(pox), posy(poy), width(wih), height(heit), path(path),headSize(0), parsedMusicPath(parseMusicPath(path))
{
...
void Box::render()
{
    ...
    DrawText(get<2>(parsedMusicPath).c_str(), posx, posy, 10, BLACK);
}
void Box::process()
{
